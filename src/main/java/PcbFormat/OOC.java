...
public class OOC {
    ...
    private String[] Command2OOC(String[] commands) throws PcbParseException {
        String prefix = "/summon FallingSand ~ ~1 ~2 " +
        ...
        final String suffix = "{id:\"MinecartCommandBlock\",Command:\"" +
                "/setblock ~ ~-1 ~ minecraft:lava 15\"},{id:\"MinecartCommandBlock\"," +
                "Command:\"setblock ~ ~ ~ air 0\"}]}]}";
        List<StringBuilder> toReturn = new ArrayList<StringBuilder>();
        int length = prefix.length();
        StringBuilder returnCommand = new StringBuilder();
        returnCommand.append(prefix);
        boolean colorblackTech = false;
        ...
        for (String cmd : commands) {
            length += 39;
            if (colorblackTech) {
                if (CommandUtil.escape(CommandUtil.escape(cmd)).length() + 12 > 30000) {
                    throw new PcbParseException("单一命令过长");
                }
                length += CommandUtil.escape(CommandUtil.escape(cmd)).length() + 12;
            } else {
                if (CommandUtil.escape(cmd).length() + 12 > 30000) {
                    throw new PcbParseException("单一命令过长");
                }
                length += CommandUtil.escape(cmd).length() + 12;
            }

            if (length > 30000) {
                returnCommand.append(suffix);
                toReturn.add(new StringBuilder(returnCommand.toString()));
                returnCommand.delete(0, returnCommand.length());
                returnCommand.append(prefix);
                length = prefix.length();
            }
            returnCommand.append("{id:\"MinecartCommandBlock\",Command:\"");
            returnCommand.append(CommandUtil.escape(cmd));
            returnCommand.append("\"},");
        }
        returnCommand.append(suffix);
        toReturn.add(new StringBuilder(returnCommand.toString()));
        List<String> finalOOCs = new ArrayList<String>();

        if (colorblackTech) {
            for (StringBuilder sb : toReturn) {
                finalOOCs.add(CommandUtil.colorBlackTech(sb.toString()));
            }
        } else {
            for (StringBuilder sb : toReturn) {
                finalOOCs.add(sb.toString());
            }
        }
        return finalOOCs.toArray(new String[finalOOCs.size()]);
    }
}
